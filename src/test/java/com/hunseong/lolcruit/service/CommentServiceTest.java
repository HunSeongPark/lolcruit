...
class CommentServiceTest {
    ...
    @DisplayName("[찾을 수 없는 게시글]댓글 수정에 실패한다")
    @Test
    void edit_fail_post_not_found() {
        ...
        // when & then
        CustomException customException = assertThrows(CustomException.class,
                () -> commentService.edit(sessionUser, postId + 1, commentId, editRequestDto));
        log.info(customException.getMessage());
    }

    @DisplayName("[찾을 수 없는 댓글]댓글 수정에 실패한다")
    @Test
    void edit_fail_comment_not_found() {
